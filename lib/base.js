...
Hoek.merge(internals.Base.prototype, { //Calling to super??
    ...
    createError(teststring, value) {

        const type = teststring.split('.')[0];
        const rule = teststring.split('.')[1];

        return CustomErrors(type, rule, value);
    },
    ...
    validate(ast) {
        ...
        for (let i = 0; i < this._tests.length; ++i) {
            ...
            const result = test.method(ast);

            if (!result) {

                throw new GraphQLError(this.createError(test.name, ast.value));
            }
