...
internals.emailRegex = (email, opts) => {

    opts = opts || {};

    const base = '[^\\.\\s@][^\\s@]*(?!\\.)@[^\\.\\s@]+(?:\\.[^\\.\\s@]+)*';

    const regex = opts.strict ? new RegExp('^' + base + '$') : new RegExp(base, 'g');
...
Hoek.merge(internals.StringScalar.prototype, {
    ...
    email(opts) {
        ...
        Joi.assert(opts, internals.emailConfigSchema, new GraphQLError('email was passed bad config'));

        return this._test('string.email', (ast) => {

            const email = ast.value;

            return internals.emailRegex(email, opts) ? ast.value : null;
        });
    }
});
